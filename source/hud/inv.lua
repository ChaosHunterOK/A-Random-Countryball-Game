...
function Inventory:add(itemType, amount, itemTypes)
    amount = amount or 1
    for i, slot in ipairs(self.items) do
        if slot and slot.type == itemType then
            local stackLimit = itemTypes[itemType].stack or 1
            local available = stackLimit - slot.count
...
function Inventory:update(dt)
    for i = 1, self.maxSlots do
        ...
        if math.abs(current - target) > 0.1 then
            self.slotTimers[i] = math.min(self.slotTimers[i] + dt, self.animDuration)
            local t = self.slotTimers[i] / self.animDuration
            local eased = easeInOutBack(t)

            local startY = (self.slotTargets[i] == -10) and 0 or -10
...
function Inventory:hasFreeSlot()
    for i = 1, self.maxSlots do
        if not self.items[i] or self.items[i].count <= 0 then
            return true
        end
    end
    return false
        ...
        if mx >= x and mx <= x+barWidth and my >= y and my <= y+barHeight then
            ...
            if slot and slot.count > 0 then
                if not self.heldItem then
                    ...
                    if self.heldItem == slot.type then
                ...
                if self.heldItem then
                    local stackLimit = itemTypes[self.heldItem].stack or 1
                    if button == 1 then
                        local toPlace = math.min(self.heldCount, stackLimit)
                        self.items[i] = { type = self.heldItem, count = toPlace }
                        self.heldCount = self.heldCount - toPlace
                        if self.heldCount <= 0 then
                            self.heldItem = nil
                            self.heldCount = 0
                        end
                    elseif button == 2 then
                        self.items[i] = { type = self.heldItem, count = 1 }
                        self.heldCount = self.heldCount - 1
                        if self.heldCount <= 0 then
                            self.heldItem = nil
                            self.heldCount = 0
...
function Inventory:mousereleased(mx, my, button, itemsModule, countryball, crafting)
    ...
    if not insideInventory and itemsModule and itemsModule.dropItem then
        itemsModule.dropItem(countryball.x + 0.6, countryball.y + 0.5, countryball.z + 0.1, self.heldItem, 0)
    end
